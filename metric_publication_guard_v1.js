(function(root,factory){
  ...
  const SPRINT_EVIDENCE_TOLERANCE_SECONDS=0.001;
  ...
  function recoverLeadingContinuousIntervalSeconds(metric,rows){
    if(!metric||!finite(metric.metricCoveredSeconds)||!rows.length)return 0;
    const segments=new Set(rows.map(row=>row.segment));
    if(segments.size!==1)return 0;
    let observedBetweenSamples=0;
    for(let i=1;i<rows.length;i++){
      const dt=rows[i].time-rows[i-1].time;
      if(!(dt>0)||dt>MAX_CONTINUOUS_SPEED_GAP_SECONDS)return 0;
      observedBetweenSamples+=dt;
    }
    const missing=Number(metric.metricCoveredSeconds)-observedBetweenSamples;
    if(!(missing>0)||missing>MAX_CONTINUOUS_SPEED_GAP_SECONDS+1e-9)return 0;
    return missing;
  }

  function continuousSpeedEvidenceSeconds(metric){
    const rows=normalizedSpeedRows(metric?.speedSamples);
    if(!rows.length)return 0;
    const observed=longestContinuousSpeedEvidenceSeconds(rows);
    const leading=recoverLeadingContinuousIntervalSeconds(metric,rows);
    return +(observed+leading).toFixed(3);
  }
  ...
  function basePublicationDecision(metric,context={}){
    ...
    if(Number(metric.metricCoverage)>1)return {publishable:false,status:'INDISPONIBLE',reason:'couverture métrique invalide (> 100%)',identityQuality};
    ...
    if(metric.avgCalibrationConfidence!==undefined&&metric.avgCalibrationConfidence!==null){
      ...
      if(Number(metric.avgCalibrationConfidence)>1)return {publishable:false,status:'INDISPONIBLE',reason:'confiance de calibration invalide (> 1)',identityQuality};
    }
    ...
    if(Number(metric.defendableScore)>1)return {publishable:false,status:'INDISPONIBLE',reason:'score de preuve métrique invalide (> 1)',identityQuality};
  ...
  function speedPublicationDecision(metric,baseDecision){
    ...
    const continuousSpeedSeconds=continuousSpeedEvidenceSeconds(metric);
  ...
  function sprintDecision(metric,speedDecision){
    ...
    const qualified=fieldInvalidDecision(count,{value:metric?.sprintQualifiedSeconds},'sprintQualifiedSeconds');
    if(!qualified?.publishable)return qualified;
    ...
    const minSprintSeconds=finite(metric?.minSprintSeconds)&&Number(metric.minSprintSeconds)>0?Number(metric.minSprintSeconds):null;
    ...
    if(sprintCount>0&&minSprintSeconds!==null&&qualifiedSeconds+SPRINT_EVIDENCE_TOLERANCE_SECONDS<sprintCount*minSprintSeconds)return {...qualified,publishable:false,status:'INDISPONIBLE',reason:'preuve sprint incohérente : durée qualifiée insuffisante pour le nombre de sprints'};
  ...
  function patch(){
    ...
    Stats.buildReport=function(coreState,coreApi,projectors){
      ...
      report.metricPublicationGuard={version:'CAY_METRIC_PUBLICATION_GUARD_V1_9',minEvidenceScore:MIN_PUBLISHABLE_EVIDENCE_SCORE,minCoveredSeconds:MIN_PUBLISHABLE_COVERED_SECONDS,minContinuousSpeedEvidenceSeconds:MIN_CONTINUOUS_SPEED_EVIDENCE_SECONDS,maxContinuousSpeedGapSeconds:MAX_CONTINUOUS_SPEED_GAP_SECONDS,minSustainedMaxSpeedSeconds:MIN_SUSTAINED_MAX_SPEED_SECONDS,minSustainedMaxSpeedIntervals:MIN_SUSTAINED_MAX_SPEED_INTERVALS,maxSpeedSourceToleranceKmh:MAX_SPEED_SOURCE_TOLERANCE_KMH,sprintEvidenceToleranceSeconds:SPRINT_EVIDENCE_TOLERANCE_SECONDS,requiresSprintCountDurationConsistency:true,requiresReliablePlayerIdentity:true,requiresExplicitCalibrationConfidence:true,requiresUnitIntervalEvidence:true,principle:'publication fail-closed par champ après preuve commune identité/couverture/qualité/confiance calibration; couverture, confiance calibration et score défendable doivent rester dans [0,1]; les valeurs et échantillons vitesse non défendables restent uniquement dans les diagnostics; chaque valeur est validée structurellement sans bloquer les autres'};
      return report;
    };
