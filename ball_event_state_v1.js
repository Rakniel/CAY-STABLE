(function(root,factory){
  ...
  function analyzeBallEvents(samples,options){
    const raw=options||{};
    const minStableOwnershipSec=configured(raw.minStableOwnershipSec,.30);
    const cfg={
      ...
      minTurnoverTravelM:configured(raw.minTurnoverTravelM,.75),maxTurnoverTransitionSec:configured(raw.maxTurnoverTransitionSec,1.5),minTurnoverObservations:configured(raw.minTurnoverObservations,3)
    };
    ...
    function commitCandidate(now,rowBall){
      ...
      const stableSec=Math.max(0,now-candidateSince);
      const opponentChange=!!(stable&&!sameOwner(stable,candidate)&&stable.team!==candidate.team);
      const requiredStableSec=opponentChange?cfg.minOpponentStableOwnershipSec:cfg.minStableOwnershipSec;
      if(stableSec<requiredStableSec){
        if(opponentChange&&stableSec>=cfg.minStableOwnershipSec)opponentStabilityDeferrals+=1;
        return;
      }
      if(stable&&!sameOwner(stable,candidate)){
        if(stable.team===candidate.team){
          ...
          const travelM=transition&&transition.fromBall&&endBall?distance(transition.fromBall,endBall):0;
          const duration=transition?now-transition.startedAt:Infinity;
          const transitionObservations=transition?transition.observations||0:0;
    ...
    return {
      ...
      motionEvaluatedFrames,motionRejectedFrames,fastBallFreeFrames,opponentStabilityDeferrals,
