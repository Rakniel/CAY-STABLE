(function(root,factory){
  ...
  const DEFAULT_PITCH_LENGTH_M=finite(MetricSmoother?.DEFAULT_PITCH_LENGTH_M)&&Number(MetricSmoother.DEFAULT_PITCH_LENGTH_M)>0?Number(MetricSmoother.DEFAULT_PITCH_LENGTH_M):105;
  const DEFAULT_PITCH_WIDTH_M=finite(MetricSmoother?.DEFAULT_PITCH_WIDTH_M)&&Number(MetricSmoother.DEFAULT_PITCH_WIDTH_M)>0?Number(MetricSmoother.DEFAULT_PITCH_WIDTH_M):68;
  const qualityFromEvidenceScore=score=>score>=.8?'FIABLE':score>0?'PARTIEL':'INDISPONIBLE';
  const pitchDimension=(value,fallback)=>finite(value)&&Number(value)>0?Number(value):fallback;
  const insideMetricPitch=(p,info={})=>!!MetricSmoother&&typeof MetricSmoother.insidePitch==='function'&&MetricSmoother.insidePitch(p,pitchDimension(info.pitchLengthM,DEFAULT_PITCH_LENGTH_M),pitchDimension(info.pitchWidthM,DEFAULT_PITCH_WIDTH_M));
  ...
  function projectorPitch(entry){
    return {
      pitchLengthM:pitchDimension(entry?.pitch?.lengthM,DEFAULT_PITCH_LENGTH_M),
      pitchWidthM:pitchDimension(entry?.pitch?.widthM,DEFAULT_PITCH_WIDTH_M),
      pitchGeometryExplicit:finite(entry?.pitch?.lengthM)&&Number(entry.pitch.lengthM)>0&&finite(entry?.pitch?.widthM)&&Number(entry.pitch.widthM)>0
    };
  }
  ...
  function robustMetricForTrack(track,projectors){
    ...
    for(let i=0;i<path.length;i++){
      ...
      const item={x:Number(projected.x),y:Number(projected.y),time:Number(p.time),segment:p.segment,calibrationConfidence:info.confidence,pitchLengthM:info.pitchLengthM,pitchWidthM:info.pitchWidthM,pitchGeometryExplicit:info.pitchGeometryExplicit};
      if(!insideMetricPitch(item,info)){rejectedOutsidePitchSamples++;outsidePitchRejectedIndexes.add(i);flush();continue;}
  ...
  function patch(){
    ...
    Stats.buildReport=function(coreState,coreApi,projectors){
      ...
      report.metricQualityGuard={version:'CAY_METRIC_QUALITY_GUARD_V1_12',smoothing:'MEDIAN_3_POINTS_PAR_RUN_METRIQUE_APRES_VETO_SPIKE_BRUT',qualityPolicy:'QUALITE = COUVERTURE_METRIQUE × CONFIANCE_CALIBRATION_MOYENNE',calibrationConfidencePolicy:'CONFIANCE_EXPLICITE_REQUISE; ABSENTE_OU_INVALIDE = 0_POUR_DEFENDABILITE',projectionRejectionPolicy:'PROJECTEUR_NON_VALIDE_ET_ECHEC_DE_PROJECTION_VALIDE_SONT_AUDITES_SEPAREMENT_PAR_ECHANTILLON_ET_LE_TEMPS_ADJACENT_AFFECTE_EST_AUDITE_SANS_SORTIR_DU_DENOMINATEUR; AUCUN_REJET_NE_PEUT_CREER_DISTANCE_VITESSE_OU_SPRINT',speedSamplePolicy:'ANCRE_DEBUT_PREMIER_INTERVALLE_PAR_RUN + FINS_INTERVALLES POUR_NE_PAS_PERDRE_LE_PREMIER_DT',coveragePolicy:'TOUT_INTERVALLE_ADJACENT_DONT_LES_TIMESTAMPS_SONT_VALIDES_RESTE_DANS_LE_DENOMINATEUR; ENTREE_STRUCTURELLEMENT_INVALIDE_CHANGEMENT_PLAN_TROU_TEMPOREL_COORDONNEE_HORS_TERRAIN_ET_SPIKE_BRUT_PENALISENT_EXPLICITEMENT_LA_COUVERTURE_SANS_CREER_DE_DISTANCE',invalidPathPolicy:'ENTREE_TRAJECTOIRE_CORROMPUE = COUPURE_DE_CONTINUITE; TIMESTAMPS_VALIDES_CONSERVENT_LE_TEMPS_ELIGIBLE_MAIS_NON_DEFENDABLE; JAMAIS_DE_DISTANCE_A_TRAVERS',rawSpikePolicy:'VETO_BRUT_AVANT_LISSAGE_A_55_KMH_MAX_VIA_METRIC_MOTION_PLAUSIBILITY_V1',pitchBoundsPolicy:'INSIDE_PITCH_REUTILISE_METRIC_TRAJECTORY_SMOOTHER_AVEC_BORNE_TERRAIN_DU_PROJECTEUR_PAR_SEGMENT_SI_EXPLICITE; FALLBACK_COMPATIBILITE_105_X_68_POUR_PROJECTEUR_LEGACY_SANS_GEOMETRIE; TEMPS_ADJACENT_AUX_REJETS_HORS_TERRAIN_AUDITE_EXPLICITEMENT',sprintPolicy:'UN_SPRINT_COMPTE_SEULEMENT_APRES_1S_CONTINUE_A_AU_MOINS_25_KMH',maxMetricGapSec:MAX_METRIC_GAP_SEC,rawSpikeThresholdKmh:RAW_SPIKE_THRESHOLD_KMH,principle:'veto partagé des entrées trajectoire corrompues, changements de plan, téléports métriques bruts, projections indisponibles/échouées et coordonnées hors terrain selon la géométrie réelle du projecteur avant tout lissage, puis filtre médian local, couverture qui conserve et audite les preuves temporelles non défendables, combinaison couverture × confiance calibration et durée minimale avant de compter un sprint'};
      return report;
    };
    Stats.__cayMetricQualityGuardPatched=true;return true;
  }
  patch();
  return {smoothRun,splitRawSpikeRuns,robustMetricForTrack,metricUnavailableReason,patch,patchTeamCalibrationEvidence,projectorInfo,projectorPitch,insideMetricPitch,qualityFromEvidenceScore,SPRINT_THRESHOLD_KMH,MIN_SPRINT_SECONDS,RAW_SPIKE_THRESHOLD_KMH,MAX_METRIC_GAP_SEC,DEFAULT_PITCH_LENGTH_M,DEFAULT_PITCH_WIDTH_M};
});