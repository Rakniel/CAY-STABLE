(function(root,factory){
  ...
  function robustMetricForTrack(track,projectors){
    ...
    for(let i=0;i<path.length;i++){
      ...
      if(!projected||!finite(projected.x)||!finite(projected.y)){flush();continue;}
    ...
    const publishable=metricDt>0&&quality!=='INDISPONIBLE';
    return {metricCoverage:+coverage.toFixed(4),metricCoveredSeconds:+metricDt.toFixed(3),eligibleSeconds:+eligibleDt.toFixed(3),distanceM:publishable?+distanceM.toFixed(2):null,avgSpeedKmh:publishable&&avgSpeedKmh!==null?+avgSpeedKmh.toFixed(2):null,maxSpeedKmh:publishable?+maxSpeedKmh.toFixed(2):null,sprintCount:publishable?sprintCount:null,sprintQualifiedSeconds:publishable?+sprintQualifiedSeconds.toFixed(3):null,sprintThresholdKmh:SPRINT_THRESHOLD_KMH,minSprintDurationSeconds:MIN_SPRINT_SECONDS,rawSpikeThresholdKmh:RAW_SPIKE_THRESHOLD_KMH,maxMetricGapSec:MAX_METRIC_GAP_SEC,quality,avgCalibrationConfidence:+avgCalibrationConfidence.toFixed(4),defendableScore:+defendableScore.toFixed(4),speedSamples:publishable?speeds:[],rejectedSpeedPairs,rejectedRawSpikePairs,rejectedOutsidePitchSamples,rejectedInvalidPathSamples,rejectedInvalidPathSeconds:+rejectedInvalidPathSeconds.toFixed(3),rejectedInvalidPathIntervals,rejectedGapSeconds:+rejectedGapSeconds.toFixed(3),gapBreaks,segmentBoundarySeconds:+segmentBoundarySeconds.toFixed(3),segmentBoundaryBreaks,smoothing:'MEDIAN_3_POINTS_PAR_RUN_METRIQUE_APRES_VETO_SPIKE_BRUT',qualityPolicy:'QUALITE = COUVERTURE_METRIQUE × CONFIANCE_CALIBRATION_MOYENNE',calibrationConfidencePolicy:'CONFIANCE_EXPLICITE_REQUISE; ABSENTE_OU_INVALIDE = INDISPONIBLE_ET_AUCUNE_STATISTIQUE_PHYSIQUE_NUMERIQUE_PUBLIEE',publicationPolicy:'DISTANCE_VITESSE_SPRINTS_ET_ECHANTILLONS_VITESSE_PUBLIES_UNIQUEMENT_SI_QUALITE_DIFFERENTE_DE_INDISPONIBLE',speedSamplePolicy:'CHAQUE_RUN_EXPOSE_UNE_ANCRE_AU_DEBUT_DU_PREMIER_INTERVALLE_PUIS_LES_FINS_D_INTERVALLES_POUR_PRESERVER_LA_DUREE_REELLE_DE_PREUVE',coveragePolicy:'TOUT_INTERVALLE_CHRONOLOGIQUE_ADJACENT_AVEC_TIMESTAMPS_VALIDES_RESTE_DANS_LE_TEMPS_ELIGIBLE; ENTREE_STRUCTURELLEMENT_INVALIDE_CHANGEMENT_PLAN_TROU_TEMPOREL_COORDONNEE_HORS_TERRAIN_OU_SPIKE_BRUT_RESTE_NON_DEFENDABLE_ET_NE_CREE_JAMAIS_DE_DISTANCE',invalidPathPolicy:'ENTREE_TRAJECTOIRE_CORROMPUE = COUPURE_DE_CONTINUITE; SI_SES_TIMESTAMPS_ADJACENTS_SONT_VALIDES_LE_TEMPS_RESTE_ELIGIBLE_MAIS_NON_DEFENDABLE; JAMAIS_PROJETEE_JAMAIS_RELIEE_JAMAIS_COMPTEE_COMME_DISTANCE',rawSpikePolicy:'VETO_AVANT_LISSAGE_SI_VITESSE_BRUTE_SUPERIEURE_A_55_KMH; LE_LISSAGE_NE_PEUT_PAS_MASQUER_UN_TELEPORT_METRIQUE',pitchBoundsPolicy:'REUTILISE_METRIC_TRAJECTORY_SMOOTHER_INSIDE_PITCH_105_X_68; AUCUNE_DISTANCE_VITESSE_OU_SPRINT_A_TRAVERS_COORDONNEE_HORS_TERRAIN',sprintContinuityPolicy:'EPISODE >= 1S A >=25KMH; RESET_SUR_CUT_SEGMENT_GAP_TEMPOREL_SPIKE_BRUT_PAIRE_REJETEE_OU_RUN_METRIQUE'};
  }
  ...
  function patch(){
    ...
    Stats.buildReport=function(coreState,coreApi,projectors){
      ...
      for(const p of report.players||[]){
        ...
        const metric=robustMetricForTrack(raw,projectors||{});p.metric={...metric,reason:metric.quality==='INDISPONIBLE'?'preuve métrique insuffisante ou confiance de calibration absente':null};
      ...
      const measured=(report.players||[]).filter(p=>p.metric?.quality!=='INDISPONIBLE'&&p.metric?.distanceM!==null),all=report.players||[];
      ...
      report.metricQualityGuard={version:'CAY_METRIC_QUALITY_GUARD_V1_9',smoothing:'MEDIAN_3_POINTS_PAR_RUN_METRIQUE_APRES_VETO_SPIKE_BRUT',qualityPolicy:'QUALITE = COUVERTURE_METRIQUE × CONFIANCE_CALIBRATION_MOYENNE',calibrationConfidencePolicy:'CONFIANCE_EXPLICITE_REQUISE; ABSENTE_OU_INVALIDE = INDISPONIBLE_ET_AUCUNE_STATISTIQUE_PHYSIQUE_NUMERIQUE_PUBLIEE',publicationPolicy:'DISTANCE_VITESSE_SPRINTS_ET_ECHANTILLONS_VITESSE_PUBLIES_UNIQUEMENT_SI_QUALITE_DIFFERENTE_DE_INDISPONIBLE',speedSamplePolicy:'ANCRE_DEBUT_PREMIER_INTERVALLE_PAR_RUN + FINS_INTERVALLES POUR_NE_PAS_PERDRE_LE_PREMIER_DT',coveragePolicy:'TOUT_INTERVALLE_ADJACENT_DONT_LES_TIMESTAMPS_SONT_VALIDES_RESTE_DANS_LE_DENOMINATEUR; ENTREE_STRUCTURELLEMENT_INVALIDE_CHANGEMENT_PLAN_TROU_TEMPOREL_COORDONNEE_HORS_TERRAIN_ET_SPIKE_BRUT_PENALISENT_EXPLICITEMENT_LA_COUVERTURE_SANS_CREER_DE_DISTANCE',invalidPathPolicy:'ENTREE_TRAJECTOIRE_CORROMPUE = COUPURE_DE_CONTINUITE; TIMESTAMPS_VALIDES_CONSERVENT_LE_TEMPS_ELIGIBLE_MAIS_NON_DEFENDABLE; JAMAIS_DE_DISTANCE_A_TRAVERS',rawSpikePolicy:'VETO_BRUT_AVANT_LISSAGE_A_55_KMH_MAX_VIA_METRIC_MOTION_PLAUSIBILITY_V1',pitchBoundsPolicy:'BORNE_TERRAIN_PARTAGEE_VIA_METRIC_TRAJECTORY_SMOOTHER_INSIDE_PITCH_105_X_68',sprintPolicy:'UN_SPRINT_COMPTE_SEULEMENT_APRES_1S_CONTINUE_A_AU_MOINS_25_KMH',maxMetricGapSec:MAX_METRIC_GAP_SEC,rawSpikeThresholdKmh:RAW_SPIKE_THRESHOLD_KMH,principle:'aucune statistique physique numérique sans preuve de calibration explicite; veto partagé des entrées trajectoire corrompues, changements de plan, téléports métriques bruts et coordonnées hors terrain avant tout lissage, puis filtre médian local, couverture qui conserve les preuves temporelles non défendables, combinaison couverture × confiance calibration et durée minimale avant de compter un sprint'};
      return report;
    };
    Stats.__cayMetricQualityGuardPatched=true;return true;
  }
  patch();
  return {smoothRun,splitRawSpikeRuns,robustMetricForTrack,patch,patchTeamCalibrationEvidence,qualityFromEvidenceScore,SPRINT_THRESHOLD_KMH,MIN_SPRINT_SECONDS,RAW_SPIKE_THRESHOLD_KMH,MAX_METRIC_GAP_SEC};
});
