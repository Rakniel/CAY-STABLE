...
2. **Very-fast-ball ownership veto** — default observed ball speed > 30 m/s keeps the ball observable but `FREE`, preventing a clearly fast pass/shot from being credited as controlled possession merely because it crosses a player's proximity radius. The threshold remains configurable for representative-footage tuning.
...
- avoids silently crediting very-high-speed pass-through frames as player possession;
...
- Expected measurable impact: lower false possession time and fewer false owner transitions on ball-detector jumps / very fast pass-throughs, while preserving observed FREE-ball coverage.
...
- Development started with a 12 m/s default ownership threshold. Full CAY non-regression showed it delayed receiver acquisition in an existing valid synthetic pass. Raising it to 22 m/s fixed that case but a second valid transition around 25 m/s still regressed. The default is therefore **30 m/s**. Existing regression expectations were preserved rather than weakened. Stricter thresholds remain configurable for measured CAY-footage tuning.
...
- `tests/ball_motion_ownership_nonregression.js` covers default very-fast pass-through ownership veto, configurable stricter gating, implausible motion rejection, normal controlled-ball compatibility, same-plan pass detection and camera-plan boundary isolation.
- Both threshold corrections came from existing non-regression failures; no historical test was relaxed to force a green build.
