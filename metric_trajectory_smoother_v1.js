(function(root,factory){
  ...
  function usableWindow(points,index,maxGapSec,maxSpacingRatio){
    ...
    if(!(min>0)||max/min>maxSpacingRatio)return null;
    return w;
  }

  function smoothSeries(points,options){
    const cfg={maxGapSec:1,maxSpacingRatio:1.35,...(options||{})};
    const src=Array.isArray(points)?points:[];
    let smoothedSamples=0;
    const out=src.map((p,i)=>{
      if(!p||!finite(p.x)||!finite(p.y)||!finite(p.time))return p?{...p}:null;
      const w=usableWindow(src,i,cfg.maxGapSec,cfg.maxSpacingRatio);
    ...
    return {
      ...
      policy:'AUCUN_LISSAGE_A_TRAVERS_COUPE_PLAN_GAP_SUPERIEUR_A_1S_OU_ECHANTILLONNAGE_IRREGULIER'
    };
  }
