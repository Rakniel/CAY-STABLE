...
- Safety policy: source/license/revision provenance is mandatory. External artifacts are fail-closed against an explicit permissive-license allowlist (MIT, Apache-2.0, BSD-2-Clause, BSD-3-Clause, ISC, CC0-1.0, Unlicense); unknown, proprietary, mixed-with-unknown, GPL and AGPL declarations are rejected. CAY-native producers must explicitly declare `kind: internal` plus `license: CAY-INTERNAL`. Samples are isolated by camera segment and calibration-anchor time; stale or missing samples return unavailable; propagation still passes every existing transform plausibility, support, inlier, residual, forward/backward and age guard before metres are exposed.
- Test: `tests/camera_motion_artifact_provider_nonregression.js` covers permissive-license acceptance, GPL/unknown/proprietary/mixed-license rejection, explicit CAY-internal provenance, ordering, freshness, segment/anchor isolation and inverse camera-motion projection against a validated absolute calibration anchor.
...
- Expected benefit: moving-camera footage can reuse one validated absolute calibration over short, evidenced motion intervals instead of forcing continuous manual calibration, while still failing closed whenever evidence or legal provenance is weak. No accuracy percentage is claimed until representative C.A. Yenne video benchmarks are run.
