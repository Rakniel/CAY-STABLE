...
(async()=>{
  ...
  ok(G&&G.version==='1.1.0','API garde licence disponible');
  ok(G.inspectLicense('MIT').allowed===true,'MIT autorisee');
  ok(G.inspectLicense('Apache-2.0').allowed===true,'Apache-2.0 autorisee');
  ok(G.inspectLicense('Proprietary').allowed===false,'licence proprietaire refusee');
  ok(G.inspectLicense('Unknown').allowed===false,'licence inconnue refusee');
  ok(G.inspectLicense('Apache-2.0 AND Unknown').allowed===false,'expression mixte inconnue refusee');
