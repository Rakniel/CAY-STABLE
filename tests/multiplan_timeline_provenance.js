...
const report=b.report({2:{segment:2,validated:true,source:'manual_4_points',confidence:.94,project:p=>({x:p.x*105,y:p.y*68})}});
...
const wrongSegment=b.report({2:{segment:1,validated:true,source:'copied_previous_plan',confidence:.99,project:p=>({x:p.x*105,y:p.y*68})}});
const wrong=wrongSegment.bridge.segments.find(s=>s.segment===2);
check('validated calibration from another segment is rejected',wrong.metricProjectionValidated===false);
check('wrong-segment rejection reason is explicit',/incompatible/.test(wrong.metricCalibrationReason||''));
const unbound=b.report({2:{validated:true,source:'legacy_validated',confidence:.99,project:p=>({x:p.x*105,y:p.y*68})}});
check('validated calibration without explicit segment binding is rejected',unbound.bridge.segments.find(s=>s.segment===2).metricProjectionValidated===false);
