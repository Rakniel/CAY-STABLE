...
ok(participation.boundaryPolicy==='HALF_OPEN_SUBSTITUTION_WINDOWS_[START,END)','substitution boundary policy is explicit');
ok(app.isPlayerActiveAt(participation,'11',29999)===true,'starter active before exit');
ok(app.isPlayerActiveAt(participation,'11',30000)===false,'starter inactive exactly at exit timestamp');
ok(app.isPlayerActiveAt(participation,'12',30000)===true,'incoming substitute active exactly at entry timestamp');
...
const activeAtFirstSub=team.roster.filter(player=>app.isPlayerActiveAt(participation,player.id,30000)).map(player=>player.id);
ok(activeAtFirstSub.length===11,'exact substitution timestamp never exposes more than 11 active players');
ok(!activeAtFirstSub.includes('11')&&activeAtFirstSub.includes('12'),'substitution timestamp belongs only to incoming player');
...
ok(starterSplit.acceptedObservations===1,'starter track excludes the exact substitution timestamp');
ok(starterSplit.rejectedObservations===2,'starter track rejects boundary and post-exit observations');
eq(starterSplit.windows[0].track.fullPath.map(p=>p.time),[29.5],'starter metric evidence uses a half-open participation window');
ok(starterSplit.boundaryPolicy==='HALF_OPEN_SUBSTITUTION_WINDOWS_[START,END)','track evidence exposes the same boundary policy');
const incomingTrack={globalId:12,fullPath:[{time:30,segment:1,x:.2,y:.2},{time:59.999,segment:1,x:.3,y:.3},{time:60,segment:1,x:.4,y:.4}]};
const incomingSplit=app.splitTrackEvidenceByParticipation(participation,'12',incomingTrack);
eq(incomingSplit.windows[0].track.fullPath.map(p=>p.time),[30,59.999],'incoming player owns entry timestamp but not next substitution timestamp');
ok(incomingSplit.acceptedObservations===2&&incomingSplit.rejectedObservations===1,'boundary observations are assigned once across successive substitutes');
