...
// L’ancien pré-troncage au score pouvait perdre les vraies détections et casser les IDs.
...
assert.strictEqual(recovered.assigned.length,11,'jamais plus de 11 CAY assignés sur une frame');
assert.deepStrictEqual(recoveredIds,initialIds,'les 11 IDs existants sont récupérés via la cascade faible');
assert.strictEqual(recovered.lowAssigned.length,11,'les détections faibles servent uniquement à récupérer les tracks existants');
assert.strictEqual(recovered.newAssigned.length,0,'aucun nouvel ID n’est créé quand les 11 places sont déjà récupérées');
assert.strictEqual(Core.summary(state).rosterTotal,11,'le clutter fort ne gonfle pas artificiellement le roster');
assert.strictEqual(state.byteTrackLowScoreRecoveries,11,'les récupérations faible confiance sont mesurables');
...
console.log('PASS ByteTrack recovery capacity non-regression: 8/8');