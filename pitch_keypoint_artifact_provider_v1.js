(function(root,factory){
  ...
  function validateArtifact(artifact){
    ...
    for(const frame of artifact.frames){
      if(!frame||!finite(frame.time)||Number(frame.time)<0||!Number.isInteger(Number(frame.segment))||Number(frame.segment)<0||!Array.isArray(frame.keypoints)){
        return {ok:false,reason:'PITCH_KEYPOINT_ARTIFACT_FRAME_INVALID'};
      }
  ...
  function createProvider(artifact,options={}){
    ...
    const maxSampleAgeSec=finite(options.maxSampleAgeSec)?Math.max(0,Number(options.maxSampleAgeSec)):
      (finite(artifact.maxSampleAgeSec)?Math.max(0,Number(artifact.maxSampleAgeSec)):.4);
    const minConfidence=finite(options.minConfidence)?clamp01(options.minConfidence):
      (finite(artifact.minConfidence)?clamp01(artifact.minConfidence):.5);
    const frames=artifact.frames.map(frame=>({
      time:Number(frame.time),segment:Number(frame.segment),keypoints:frame.keypoints.map(k=>({...k}))
    }));
