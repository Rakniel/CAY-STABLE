(function(root){
  ...
  function decode(outputs,opts={}){
    ...
    const personClassId=Number.isInteger(opts.personClassId)?opts.personClassId:0;
    ...
    if(personClassId<0||personClassId>=classes)throw new Error('RFDETR_PERSON_CLASS_OUT_OF_RANGE');
    for(let q=0;q<queries;q++){
      if(backgroundClassId===personClassId)continue;
      const score=sigmoid(logitData[q*classes+personClassId]);
      ...
      out.push({x:x1,y:y1,w:x2-x1,h:y2-y1,score,class:'person',classId:personClassId,source:'rfdetr_onnx'});
    }
    out.sort((a,b)=>b.score-a.score);
    return out.slice(0,maxBoxes);
  }
  root.CAYRFDETRONNXAdapter={decode,pickOutputs,sigmoid,license:'Apache-2.0-adaptation'};
